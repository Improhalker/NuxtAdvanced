    ...
    <nav class="fixed top-0 left-0 w-full z-50 bg-white/70 dark:bg-black/70 backdrop-blur-md shadow-md">
        <div class="max-w-6xl mx-auto px-4 py-4 flex justify-between items-center">
            ...
            <button @click="toggleMenu"
                ...
                <span :class="[
                    'block h-1 w-6 bg-accent transition-transform duration-300',
                    isMenuOpen ? 'rotate-45 translate-y-1.5' : ''
                ]"></span>
                <span :class="[
                    'block h-1 w-6 bg-accent transition-opacity duration-300',
            ...
            <!-- Desktop menu -->
            <ul class="hidden md:flex space-x-6 text-lg text-primaryText">
                <li>
        ...
        <transition name="fade-slide">
            <ul v-if="isMenuOpen"
                class="fixed inset-0 bg-background dark:bg-surface text-primaryText flex flex-col items-center justify-center space-y-6 text-2xl z-40 w-screen h-screen overflow-hidden">
                <li>
